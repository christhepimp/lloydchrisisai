...
=====================================
Decoder-only Transformer — NumPy only.
...
class MultiHeadAttention:
    ...
    def set_weights(self, d: dict):
        self.W_q, self.W_k, self.W_v, self.W_o = d["W_q"], d["W_k"], d["W_v"], d["W_o"]
...
class FeedForward:
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        self._cache_x = x
        self._cache_h = np.maximum(0, x @ self.W1 + self.b1)
...
class TransformerBlock:
    ...
    def forward(
        ...
        x = x + self.attn.forward(layer_norm(x), mask, importance_bias=importance_bias)
...
class TinyTransformer:
    def __init__(
        ...
        self.last_importance_bias: Optional[np.ndarray] = None
        ...
        for _ in range(max_new_tokens):
            ...
            if importance_bias is not None:
                ...
                if b.shape[0] >= len(window):
                    ...
                    bias = np.pad(b, (len(window) - b.shape[0], 0))
            logits = self.forward(x, importance_bias=bias)
    ...
    def set_state(self, state: dict):
        ...
        if (
            ...
            or cfg["d_model"] != self.d_model
            or cfg["n_layers"] != self.n_layers
        ):
    ...
    def save(self, path: str | Path):
        ...
        np.savez_compressed(
            ...
            b_out=state["b_out"],
            **{
                f"block_{i}_attn_W_q": b["attn"]["W_q"]
                for i, b in enumerate(state["blocks"])
            },
            **{
                f"block_{i}_attn_W_k": b["attn"]["W_k"]
                for i, b in enumerate(state["blocks"])
            },
            **{
                f"block_{i}_attn_W_v": b["attn"]["W_v"]
                for i, b in enumerate(state["blocks"])
            },
            **{
                f"block_{i}_attn_W_o": b["attn"]["W_o"]
                for i, b in enumerate(state["blocks"])
            },
            **{
                f"block_{i}_ffn_W1": b["ffn"]["W1"]
                for i, b in enumerate(state["blocks"])
            },
            **{
                f"block_{i}_ffn_b1": b["ffn"]["b1"]
                for i, b in enumerate(state["blocks"])
            },
            **{
                f"block_{i}_ffn_W2": b["ffn"]["W2"]
                for i, b in enumerate(state["blocks"])
            },
            **{
                f"block_{i}_ffn_b2": b["ffn"]["b2"]
                for i, b in enumerate(state["blocks"])
            },
        )
...
def train_step(
    ...
    logits = model.forward(batch_x, importance_bias=importance_bias)
    loss, d_logits = cross_entropy_loss_and_grad(logits, batch_y)

    d_final = d_logits @ model.W_out.T
    d_W_out = np.sum(model._cache_final.transpose(0, 2, 1) @ d_logits, axis=0)
